...
int SERVOMIN[]  {100, 108, 100}; //PLEASE ENTER: The lower motor PPM limit (Servo Min) as noted in your EDMO box
int SERVOMAX[]  {454, 490, 465}; //PLEASE ENTER: The upper motor PPM limit (Servo Max) as noted in your EDMO box

int POTIMIN[]  {121, 142, 130}; //PLEASE ENTER: The lower potentiometer limit (Poti Low) as noted in your EDMO box
int POTIMAX[]  {693, 744, 710}; //PLEASE ENTER: The upper potentiometer limit (Poti High) as noted in your EDMO box
...
double b = 1; // we assume that all oscillators use same adaptation rate for offset
...
void loop(){   
    ...
    if (currentMillis - previousMillis >= timeStep){//CPG update interval (10ms)(do NOT modify!!)
      ...
      for (int i = 0; i < 2; i++) {
        ...
        //convert to deg
        //osc[i].amplitude = osc[i].amplitude * 180 / PI;
        //osc[i].offset = osc[i].offset * 180 / PI;
        ...
        for (int j = 0; j < 2; j++) {
          if (i != j)
          {
            ...
            sum = sum + w * osc[j].amplitude * sin(osc[j].phase - osc[i].phase - osc[i].phaseBias[j]);
          }
        }
        
        // smooth modulation of phase coupling
        osc[i].rateOfPhase = 2*PI*frequency + sum;

        // smooth modulation of amplitude
        ...
        // output equation
        float theta = osc[i].amplitude * sin(osc[i].phase) + osc[i].offset;
        //Serial.print(theta);

        frequency = rateOfFrequency * (targetFrequency - frequency); // determine error
